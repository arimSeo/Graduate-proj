...
# r/p_mood에서 앞에 random_r_mood에 존재하면 리스트에서 빼내고 다음 랜덤 버튼엔 앞에꺼에서 없는 걸로 랜덤. 
    ...
    if request.POST:                #각 input에 감성 submit했을때 나올 추천리스트 다르게
        if request.POST['gam']:     #이렇게 하면 페이지 감정 수만큼 만들???/xxxx main으로 각각 다보내면 되지않나???
...
def main(request):
    ...
    food=ThreeRecommend.objects.filter(category="식당")
    cafe=ThreeRecommend.objects.filter(category="카페")
    alchol=ThreeRecommend.objects.filter(category="술집")
